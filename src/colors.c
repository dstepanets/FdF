...
int				get_pixel_color(struct s_line ln)
{
	...
	rgb[0] = light_my_fire((ln.s_color >> 16) & 0xFF,
							(ln.e_color >> 16) & 0xFF, rat);
	rgb[1] = light_my_fire((ln.s_color >> 8) & 0xFF,
							(ln.e_color >> 8) & 0xFF, rat);
...
int				get_dots_color(t_fdf *f, int z)
{
	...
	rgb[0] = light_my_fire((f->colors[0] >> 16) & 0xFF,
							(f->colors[1] >> 16) & 0xFF, rat);
	rgb[1] = light_my_fire((f->colors[0] >> 8) & 0xFF,
							(f->colors[1] >> 8) & 0xFF, rat);
