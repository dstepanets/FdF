...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/05/15 15:28:34 by dstepane          #+#    #+#             */
/*   Updated: 2019/05/15 15:28:36 by dstepane         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			rotate_x(t_fdf *f, int *y, double *z)
{
	...
	old_y = *y;
	new_y = old_y * cos(PI / 180 * f->angle_x) +
		*z * sin(PI / 180 * f->angle_x);
...
void			rotate_y(t_fdf *f, int *x, double *z)
{
	...
	old_x = *x;
	new_x = old_x * cos(PI / 180 * f->angle_y)
		+ *z * sin(PI / 180 * f->angle_y);
...
void			rotate_z(t_fdf *f, int *x, int *y)
{
	...
	old_x = *x;
	old_y = *y;
	new_x = old_x * cos(PI / 180 * f->angle_z) -
		old_y * sin(PI / 180 * f->angle_z);
	new_y = old_x * sin(PI / 180 * f->angle_z) +
		old_y * cos(PI / 180 * f->angle_z);
	*x = new_x;
	*y = new_y;
}
